...
export async function GET(req: NextRequest) {
  ...
  if (excludeCollectionId) query.collection = { ...(query.collection ?? {}), $ne: excludeCollectionId };
  if (excludeGroupId) query.group = { ...(query.group ?? {}), $nin: [excludeGroupId] };
  if (excludeFlagged === "true") query.flagged = { $ne: true };
  if (excludeTokenTypes.length) query.tokenType = { ...(query.tokenType ?? {}), $nin: excludeTokenTypes };
  if (excludeLabels.length) query.labels = { ...(query.labels ?? {}), $nin: excludeLabels };
  if (excludeComponents.length) query.associatedComponents = { ...(query.associatedComponents ?? {}), $nin: excludeComponents };
  ...
  if (themeId) {
    ...
    if (theme && !theme.isBase) {
      ...
      }).select("token lightValue darkValue").lean();
